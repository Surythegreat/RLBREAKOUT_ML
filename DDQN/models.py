...
class Model(nn.Module):
    def __init__(self, output_size=4, frame_stack_size=4):
        ...
        self.conv1 = nn.Conv2d(frame_stack_size, 32, kernel_size=(8, 8), stride=(4, 4))
        ...
        self.action_value3 = nn.Linear(1024, output_size)
    ...
    def save_the_model(self, path='DDQN/models/model.pt'):
        if not os.path.exists('models'):
            os.makedirs('models')
        torch.save(self.state_dict(), path)

    def load_the_model(self, path='DDQN/models/model.pt'):
        try:
