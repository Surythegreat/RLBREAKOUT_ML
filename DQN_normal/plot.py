...
def plot_training_data(log_file='DQN_normal/training_log.csv', output_dir='DQN_normal/plots'):
    """
    ...
    data['Smoothed Reward'] = data['Total Reward'].rolling(window=100, min_periods=1).mean()
    
    # --- FIX 2: Use 'Loss' instead of 'loss' ---
    loss_data = data[data['Loss'] > 0].copy() 
    loss_data['Smoothed Loss'] = loss_data['Loss'].rolling(window=100, min_periods=1).mean() *10
